...
A Foundry VTT (v13) module for D&D 5e 5.2.5 that tracks dice roll statistics per player, die type, action type, skill check type/save type) by session or in total. It supports both standard 5e rolls and Midi-QOL workflows and provides a polished dashboard with charts and comparison tools.
...
- GM-only tools for reset and visibility.
...
- Hidden players stats are still recorded, they are just not counted/shown.
